<template>
    <Form
        v-if="championshipCalendarForm"
        :initial-values="championshipCalendarForm"
...
import { useUpdateChampionshipCalendar } from '../../composables/update-championship-calendar.composable'
...
const championshipCalendarForm = ref({
    championshipId: props.championshipCalendar?.championshipId,
    associationId: props.championshipCalendar?.associationId,
    roundNumber: props.championshipCalendar?.roundNumber,
    eventName: props.championshipCalendar?.eventName,
    startDate: toDate(props.championshipCalendar?.startDate),
    endDate: toDate(props.championshipCalendar?.endDate),
    status: props.championshipCalendar?.status,
})

const {
    championshipCalendar: championshipCalendarUpdated,
    updateChampionshipCalendar,
    loading,
    errorState,
} = useUpdateChampionshipCalendar({
    onError: (title, error) => {
        toast.add({
            severity: 'error',
            summary: title,
...
const onSubmit = async (values, { setTouched, setFieldError }) => {
    ...
    await updateChampionshipCalendar(props.championshipCalendar.id, serializeValues(values))

    if (championshipCalendarUpdated.value?.id) {
        toast.add({
            severity: 'success',
            summary: 'Exito',
            detail: `Fecha del calendario modificada correctamente ${championshipCalendarUpdated.value.eventName}`,
            life: 3000,
        })
        emit('updated', championshipCalendarUpdated.value)
        return
    }
